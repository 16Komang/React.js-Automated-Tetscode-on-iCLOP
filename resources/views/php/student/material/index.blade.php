                        ...
                        <li class="nav-item">
                            ...
                            <li class="nav-item">
                                <div class="row align-items-start">
                                    ...
                                    <div class="col">
                                        <a class="nav-link" href="#" onclick="showContent('validation')" style="color: #34364A;" id="validationLink">Student Submission</a>
                                    </div>
                                </div>
                            </li>
                        @if($role == "teacher")
                            ...
                            <!--        </div>-->
                            <!--        <div class="col">-->
                            <!--            <a class="nav-link" href="#" onclick="showContent('rank')" style="color: #34364A;" id="rankLink">Student Report</a>-->
                            <!--        </div>-->
                            <!--    </div>-->
                            <!--</li>-->
            ...
            <main class="col-md-9  mb-5">
                            ...
                            <div class="tab-content mt-3">
                                ...
                                <div class="tab-pane fade" id="finished">
                                    ...
                                    @if($role == "student")
                                     <div class="table-responsive mb-5">
                                        <table class="table table-striped" id="topicFinished">
                                            <thead>
                                            <tr>
                                ...
                                <div class="tab-pane fade" id="progress">
                                    ...
                                    <p>This is the content for the Topic Finished.</p>
                                     <div class="table-responsive mb-5">
                                    <table class="table"  id="progressFinished">
                                        <thead>
                                        <tr>
                                            <th>User Name</th>
                                            <th>Progress (%)</th>
                                        ...
                                        @foreach($progress as $userId => $userProgress)
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="progress">
                                                        <div class="progress-bar" role="progressbar" style="width: {{ $userProgress['percent'] }}%;" aria-valuenow="{{ $userProgress['percent'] }}" aria-valuemin="0" aria-valuemax="100">{{ $userProgress['percent'] }}%</div>
                                                    </div>
                                                </td>
                                            </tr>
                                        @endforeach
                    ...
                    <table class="table"  id="studentSubmissionTable"  >
                        ...
                        @if($role == "teacher")
                            @php
                                ...
                                $studentSubmissions = DB::table('php_user_submits')
                                               ->join('users', 'users.id', '=', 'php_user_submits.user_id')
                                               ->join('php_topics_detail', 'php_topics_detail.id', '=', 'php_user_submits.topics_id')
                                               ->select('php_user_submits.created_at as Time', 'users.name as UserName', 'php_topics_detail.description as TopicName', 'php_user_submits.score as Score')
                                               ->get();
                            @endphp
                ...
                @if($role == "teacher")
                    <div id="rank" class="content" style="display: none;">
                        ...
                        @php
                            // Fetch all topics
        $topics = DB::table('php_topics_detail')->orderBy('title')->where('flag', '1')->get();

        // Fetch all users and their scores per topic
        $usersScores = DB::table('php_student_rank as r')
                         ->join('users as u', 'r.id_user', '=', 'u.id')
                         ->select('u.id as UserId', 'u.name as UserName', 'r.topics_id', 'r.best_score')
                         ->get();

        // Organize scores by user and topic
        $reportData = [];
        foreach ($usersScores as $score) {
            $reportData[$score->UserName][$score->topics_id] = $score->best_score;
        }

        // Ensure all topics are accounted for each user
        $allUsers = DB::table('users')->select('id', 'name')->get();
        $topicNames = $topics->pluck('title', 'id')->all();  // Associative array of topic id => topic title

        // Build a complete matrix of user/topic scores, filling missing data with 'N/A'
        $fullReportData = [];
        foreach ($allUsers as $user) {
            foreach ($topicNames as $topicId => $topicTitle) {
                $fullReportData[$user->name][$topicTitle] = $reportData[$user->name][$topicId] ?? 'N/A';
            }
        }

                        @endphp
 <div class="table-responsive mb-5">
                        <table class="table table-bordered" id="tableStudentReport">
                            <thead>
                            <tr>
                                <th>User / Topic</th>
                            ...
                            @php
                                ...
                                @endphp
                            @foreach ($reportData as $userName => $scores)
                                <tr>
                                    <td>{{ $userName }}</td>
     ...
     <script>
        $(document).ready(function() {
            ...
            $('#progressTable').DataTable({
                ...
                buttons: [
                    {
                        ...
                        filename: 'php_data_export_topic_finished_student_' + new Date().toLocaleDateString() + '_' + new Date().toLocaleTimeString(),
                ...
                text: 'Export to PDF',
                title: 'Data Export REACT',
                filename: 'php_data_export_topic_finished_student_' + new Date().toLocaleDateString().replace(/\//g, '-') + '_' + new Date().toLocaleTimeString().replace(/:/g, '-'),
                ...
                buttons: [
                    {
                        ...
                        text: 'Export to Excel',
                        title: 'Data Export REACT',
                        filename: 'php_data_export_student_submission_student_' + new Date().toLocaleDateString() + '_' + new Date().toLocaleTimeString(),
                        customize: function(xlsx) {
                            var sheet = xlsx.xl.worksheets['sheet1.xml'];
                            // Customizations go here
                        }
                    ...
                    text: 'Export to PDF',
                    title: 'Data Export REACT',
                    filename: 'php_data_export_student_submission_student_' + new Date().toLocaleDateString().replace(/\//g, '-') + '_' + new Date().toLocaleTimeString().replace(/:/g, '-'),
                    orientation: 'portrait', // 'portrait' or 'landscape'
                    pageSize: 'A4', // 'A3', 'A4', 'A5', 'LEGAL', 'LETTER' or 'TABLOID'
                    exportOptions: {
                        columns: ':visible' // Export visible columns only
                ...
                buttons: [
                    {
                        ...
                        text: 'Export to Excel',
                        title: 'Data Export REACT',
                        filename: 'php_data_export_progress_student_' + new Date().toLocaleDateString() + '_' + new Date().toLocaleTimeString(),
                        customize: function(xlsx) {
                            var sheet = xlsx.xl.worksheets['sheet1.xml'];
                            // Customizations go here
                        }
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        text: 'Export to Excel',
                        title: 'Data Export PHP',
                        filename: 'php_data_topicFinished_student_' + new Date().toLocaleDateString() + '_' + new Date().toLocaleTimeString(),
                        customize: function(xlsx) {
                            var sheet = xlsx.xl.worksheets['sheet1.xml'];
                            // Customizations go here
                        }
